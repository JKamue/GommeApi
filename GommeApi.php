...
class GommeApi
{
    ...
    public static function convertTagToName($tag) {
        $key = stringIsolateBetween(stringReplaceBreaks(get_contents("https://www.gommehd.net/")), "<meta name=\"csrf-token\" content=\"","\"> <title>");;
        // Generated by curl-to-PHP: http://incarnate.github.io/curl-to-php/
        $ch = curl_init();

        curl_setopt($ch, CURLOPT_URL, 'https://www.gommehd.net/clans/search-clan');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, "_csrf=".urlencode($key)."&search=%7B%22page%22%3A+1%2C+%22gamemode%22%3A+%22null%22%2C+%22type%22%3A+%22CLAN_TAG%22%2C+%22query%22%3A+%22".urldecode($tag)."%22%2C+%22filters%22%3A%5B%5D%7D");
        curl_setopt($ch, CURLOPT_ENCODING, 'gzip, deflate');

        $headers = array();
        $headers[] = 'Authority: www.gommehd.net';
        $headers[] = 'Cache-Control: max-age=0';
        $headers[] = 'Upgrade-Insecure-Requests: 1';
        $headers[] = 'Origin: https://www.gommehd.net';
        $headers[] = 'Content-Type: application/x-www-form-urlencoded';
        $headers[] = 'User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/81.0.4015.0 Safari/537.36';
        $headers[] = 'Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9';
        $headers[] = 'Sec-Fetch-Site: same-origin';
        $headers[] = 'Sec-Fetch-Mode: navigate';
        $headers[] = 'Sec-Fetch-User: ?1';
        $headers[] = 'Sec-Fetch-Dest: document';
        $headers[] = 'Referer: https://www.gommehd.net/clans';
        $headers[] = 'Accept-Language: de-DE,de;q=0.9,en-US;q=0.8,en;q=0.7';
        $headers[] = 'Cookie: __cfduid=dcac5296911e789079b395f78f155790f1526133115; _ga=GA1.2.1786320754.1526133118; _gid=GA1.2.263680115.1526133118; _csrf=fe54b1113c9727b7bba329be37318397bc617fd0366be47718ad9f191353a486a%3A2%3A%7Bi%3A0%3Bs%3A5%3A%22_csrf%22%3Bi%3A1%3Bs%3A32%3A%228Dy0kKFMXWD25PX1yGXJalC9EOWRFquY%22%3B%7D';
        curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);

        $result = stringReplaceBreaks(curl_exec($ch));
        if (curl_errno($ch)) {
            echo 'Error:' . curl_error($ch);
        }

        curl_close($ch);
        $stripped = stringIsolateBetween($result,"<ul class=\"clansSearchResults list-group\">"," </li> </ul> <div class=\"text-center\">");
        $data = explode( "<li class=\"list-group-item\">", $stripped);

        foreach ($data as &$clan)
        {
            $foundTag = stringIsolateBetween($clan, "[","]");
            if ($foundTag == $tag){
                return stringIsolateBetween($clan, "<a href=\"/clan-profile?name=", "\">");
            }
        }
